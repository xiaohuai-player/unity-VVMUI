...
namespace VVMUI.Core.Data {
	...
	public sealed class DictionaryData {
		public static DictionaryData<T> Parse<T> (object data) where T : IData {
			return (DictionaryData<T>) Parse (typeof (T), data);
		}
		...
		public void Parse (object data) {
			...
			foreach (string key in dict.Keys) {
				if (this.ContainsKey (key) && this [key] != null) {
					if (isList) {
						(this [key] as IListData).Parse (dict[key]);
					} else if (isDict) {
						(this [key] as IDictionaryData).Parse (dict[key]);
					} else if (isStruct) {
